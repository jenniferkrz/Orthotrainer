    ...
    function loadBasalMusterlösungen() {
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                // Setze basale Werte UND Oberkiefer dental direkt in die Skalen
                result.forEach(item => {
                    ...
                    } else if (item.input_id === "midline_ok_dental") {
                        // NEU: Oberkiefer dental setzen
                        console.log("OK Dental gefunden:", item.value);
                        $("#midline_ok_dental").val(item.value);
                        console.log("OK Dental Input gesetzt auf:", $("#midline_ok_dental").val());

                        const $scale = $("#scale_ok_dental");
                        const value = parseFloat(item.value);
                        const min = -10, max = 10;
                        const span = max - min;
                        const pct = ((value - min) / span) * 96 + 2;

                        console.log(`OK Dental - Wert: ${value}, Position: ${pct}%`);
                        $scale.find('.ml-indicator').css('left', pct + '%');
                        setTimeout(() => disableScale('scale_ok_dental'), 100);

...
function initReviewUI() {
    ...
    const reviewStyles = `
        <style id="review_styles">
            ...
            #review_stats .stat-value.empty { color: #ff9800; }
        </style>
    `;
...
function applyReviewData(data) {
    ...
    data.forEach(function(item) {
        ...
        if (isEmpty) {
            $container.addClass('review_empty');
            stats.empty++;
        } else if (isCorrect) {
...
function applyRadioReview(item, stats) {
    ...
    const $questionContainer = $radioGroup.first().closest('.field_group, .checkbox_group').parent();
